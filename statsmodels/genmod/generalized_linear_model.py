...
class GLM(base.LikelihoodModel):
    ...
    def _fit_irls(self, start_params=None, maxiter=100, tol=1e-8,
        ...
        maxiter = 20
        GLMResultsWrapper_last_iter = None
        for iteration in tqdm(range(maxiter), desc="IRLS", total=maxiter):
            try:
                ...
                GLMResultsWrapper_last_iter = GLMResultsWrapper(glm_results)
                if converged:
                    return GLMResultsWrapper_last_iter
            except Exception as e:
                print("Overflow occurred in fir IRLS at iteration", iteration, "return last suitable iteration", "error: ", e)
                return GLMResultsWrapper_last_iter
