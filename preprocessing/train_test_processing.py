...
def flow_train_test_split(flow_df, population_df, washington_crs, grid, experiment_id = "0"):
    """
    ...
    train_set[['region_index']].to_csv(f'../processed_data/{experiment_id}/train_region_index.csv', index=False)
    train_set[['region_index']].to_csv(f'../processed_data/{experiment_id}/test_region_index.csv', index=False)

    return train_set[['region_index', 'census_tracts_geoids']], test_set[['region_index', 'census_tracts_geoids']]


def filter_flow_data(flow_df, population_df, train_set, test_set, experiment_id = "0"):
    """
    ...
    train_set_adjusted[['geoid_o', 'geoid_d', 'pop_flows']].to_csv(f'../processed_data/{experiment_id}/train_flows.csv', index=False)
    test_set_adjusted[['geoid_o', 'geoid_d', 'pop_flows']].to_csv(f'../processed_data/{experiment_id}/test_flows.csv', index=False)
