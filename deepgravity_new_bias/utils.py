...
def _is_support_files_computed(db):
    if os.path.isdir(db + '/processed'):
        base = db + '/processed/'
        return os.path.isfile(base + 'tileid2oa2handmade_features.json') and os.path.isfile(
            base + 'oa_gdf.csv.gz') and os.path.isfile(base + 'flows_oa.csv.zip') and os.path.isfile(
            base + 'msoa_df_all.csv.zip') and os.path.isfile(base + 'oa2features.pkl') and os.path.isfile(
            base + 'oa2centroid.pkl')
    else:
        return False
...
def _check_base_files(db_dir):
    ...
    if not (os.path.isfile(db_dir + '/output_areas.shp') or os.path.isfile(db_dir + '/output_areas.geojson')):
        raise ValueError(
            'Output areas are missing! There must be a file named output_areas.shp or output_areas.geojson')
...
@deprecated
def _compute_support_files(db_dir, tile_id_column, tile_geometry, oa_id_column, oa_geometry, flow_origin_column,
                           flow_destination_column, flow_flows_column):
    ...
    flows = pd.read_csv(db_dir + '/flows.csv',
                        dtype={flow_origin_column: str, flow_destination_column: str, flow_flows_column: int})
    flows = flows[[flow_origin_column, flow_destination_column, flow_flows_column]]

    flows.rename(
        columns={flow_origin_column: 'residence', flow_destination_column: 'workplace', flow_flows_column: 'commuters'},
        inplace=True)
...
def load_data(db_dir, dataset_dir, tile_id_column, tile_geometry, oa_id_column, oa_geometry, flow_origin_column,
              flow_destination_column, flow_flows_column):
    ...
    with open(os.path.join(db_dir, dataset_dir, 'tileid2oa2handmade_features.json')) as f:
        tileid2oa2features2vals = json.load(f)

    # oa_gdf
    oa_gdf = pd.read_csv(str(os.path.join(db_dir, dataset_dir, 'oa_gdf.csv.gz')), dtype={'geo_code': 'str'})

    # flow_df
    flow_df = pd.read_csv(str(os.path.join(db_dir, dataset_dir, 'flows_oa.csv.zip')), \
    ...
    with open(os.path.join(db_dir, dataset_dir, 'oa2features.pkl'), 'rb') as f:
        oa2features = pickle.load(f)

    with open(os.path.join(db_dir, dataset_dir, 'od2flow.pkl'), 'rb') as f:
        od2flow = pickle.load(f)

    with open(os.path.join(db_dir, dataset_dir, 'oa2centroid.pkl'), 'rb') as f:
...
def instantiate_model(oa2centroid, oa2features, oa2pop, dim_input, device=torch.device("cpu"), dim_hidden=256, lr=5e-6,
